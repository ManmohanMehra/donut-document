...
augment = A.Compose([
    # --- Geometric (Mild, does not affect text legibility) ---
    ...
    # --- Mutually Exclusive Heavy Degradations ---
    # Apply at most ONE degradation per image, with milder parameters
    A.OneOf([
        A.GaussNoise(std_range=(0.01, 0.1), p=1.0),
        A.ISONoise(color_shift=(0.01, 0.03), intensity=(0.1, 0.3), p=1.0),
        A.MotionBlur(blur_limit=3, p=1.0),
        A.Downscale(scale_range=(0.7, 0.9), p=1.0),          # Don't decimate the resolution
        A.ImageCompression(quality_range=(75, 100), p=1.0),  # Moderately high quality
        A.RandomFog(fog_coef_range=(0.05, 0.15), p=1.0),     # Mild glare/fog
    ], p=0.6),  # 60% chance to get one degradation, 40% chance to get none
])
